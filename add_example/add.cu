...
int main(void)
{
    ...
    // first 1 in <<<1, 1>>> is the number of blocks
    // second 1 in <<<1, 1>>> is the number of threads per block
    // add<<<1, 1>>>(N, x, y); // Duration: 128.28 ms
    // add_parallel_one_block<<<1, 256>>>(N, x, y); // Duration: 1.49 ms
    add_parallel_multiple_blocks<<<1, 256>>>(N, x, y); // Duration: 1.49 ms
